...
def center_crop(x, crop_h, crop_w,
    ...
    return np.array(im.resize([64, 64], Image.BILINEAR))


def transform(image, input_height, input_width,
              resize_h=64, resize_w=64, crop=True):
    if crop:
    ...
    im = Image.fromarray(
        np.uint8(image[j:j+input_height, i:i+input_width]))
    return np.array(im.resize([64, 64], Image.BILINEAR))/127.5 - 1.
